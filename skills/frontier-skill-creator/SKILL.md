...
Use the currently installed official `$skill-creator` for current packaging, scaffolding, metadata, and validator mechanics when available. This skill governs content design. Do not modify or shadow a `.system` skill by default; create a distinctly named personal or project skill so official updates remain available.
...
- Treat `.system` and plugin-managed skills as updateable upstream assets. Preserve them and prefer a personal overlay, complementary skill, or distinct fork unless the user explicitly accepts overwrite and collision risk.
...
- Confirm folder name, frontmatter, direct reference paths, scripts, assets, and `agents/openai.yaml` against the current official format.
